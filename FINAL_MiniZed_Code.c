...
void pollForAccel()
{
		...
		ACC_VAL[0]=(iacceleration_X*4*0.061)/1000;
		ACC_VAL[1]=(iacceleration_Y*4*0.061)/1000;
		ACC_VAL[2]=(iacceleration_Z*4*0.061)/1000;
...
static int RecvFrame(XCanPs *InstancePtr) {
	...
	if(ID == 1531 || ID == 1532){
		...
		txdata[7]=d7;
		txdata[8]=d8;
		XUartPs_Send(&Uart_PS_0, txdata, 10);
	}
	if(ID == 1536){
		msg++;
		ACC_VAL[0] = 0;
		...
		ACC_VAL[2] = 0;
		ACC_VAL[3] = 0;
		pollForAccel();		//Getting Accelerometer Values
		Buf[msg][0]=10;		// ID For AccelValues
		Buf[msg][1]=6;
		ACC_VAL[0]=ACC_VAL[0]*1000;
		ACC_VAL[1]=ACC_VAL[1]*1000;
		ACC_VAL[2]=ACC_VAL[2]*1000;
		Buf[msg][2] = *((uint8_t*) &(ACC_VAL[0]) + 1);
		Buf[msg][3] = *((uint8_t*) &(ACC_VAL[0]) + 0);
		Buf[msg][4] = *((uint8_t*) &(ACC_VAL[1]) + 1);
		Buf[msg][5] = *((uint8_t*) &(ACC_VAL[1]) + 0);
		Buf[msg][6] = *((uint8_t*) &(ACC_VAL[2]) + 1);
		Buf[msg][7] = *((uint8_t*) &(ACC_VAL[2]) + 0);
		}
	return Status;
}
